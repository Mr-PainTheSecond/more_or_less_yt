...
void handleMouseClick(SDL_MouseButtonEvent button, bool* gameRunning, int* timeClocked, int* counter) {
		...
		while (SDL_PollEvent(&event)) {
			...
			if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN && canClick) {
				if (event.button.button == SDL_BUTTON_LEFT) {
					...
					cooldown = clock();
					// When state is normal, we will presses as a guess
					handleMouseClick(event.button, &gameRunning, &timeClocked, &counter);
				}
			}
